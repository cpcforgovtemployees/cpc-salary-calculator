...
/**
 * src/app/nps-calculator/page.tsx
 * National Pension System (NPS) Calculator — Govt Employees
 * App Router + Tailwind (matches your DA Arrears page style)
 * Tabs: Underline style (Monthly NPS Deduction | NPS Retirement Projection)
 * Currency rule: Show ₹ only on MAIN results (totals, corpus, pension)
 * Print button at bottom (emoji icon) using window.print()
 */
...
// Govt rules
const EMP_PCT = 0.10; // Employee 10%
const EMPR_PCT = 0.14; // Employer 14%
...
export default function Page() {
  ...
  // UI state
  const [tab, setTab] = useState<"deduction" | "projection">("deduction");
  ...
  // Monthly NPS (button-gated display like your DA page)
  ...
  const [annualReturnPct, setAnnualReturnPct] = useState("9"); // user enters manually
  const [currentCorpus, setCurrentCorpus] = useState("0");
  const [annuityPurchasePct, setAnnuityPurchasePct] = useState("40"); // user can change
  const [annuityRatePct, setAnnuityRatePct] = useState("6"); // annual annuity rate
  ...
  const annuityPurchase = useMemo(() => totalCorpusAtRetirement * (clamp(toNum(annuityPurchasePct), 0, 100) / 100), [
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <main className="flex-1 container mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
          ...
          <div className="flex gap-6 border-b border-gray-200">
            {[
              ...
              <button
                ...
                className={`relative -mb-px pb-3 text-sm sm:text-base font-medium border-b-2 transition-colors ${
                  tab === t.key
        ...
        {tab === "projection" && (
          <Card className="max-w-3xl mx-auto p-6 sm:p-8 border border-gray-200 rounded-xl shadow-sm bg-white space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <Label className="font-medium text-gray-700">Basic Pay</Label>
                <Input type="number" value={basic} onChange={(e) => setBasic(e.target.value)} placeholder="e.g. 60000" className="mt-1" />
              </div>
              <div>
                <Label className="font-medium text-gray-700">DA (%)</Label>
                <Input type="number" value={daPct} onChange={(e) => setDaPct(e.target.value)} placeholder="e.g. 50" className="mt-1" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input type="number" value={retAge} onChange={(e) => setRetAge(e.target.value)} className="mt-1" />
              </div>
              <div>
                <Label className="font-medium text-gray-700">Expected Annual Return (%)</Label>
                <Input type="number" value={annualReturnPct} onChange={(e) => setAnnualReturnPct(e.target.value)} placeholder="e.g. 9" className="mt-1" />
              </div>
              <div>
                <Label className="font-medium text-gray-700">Current NPS Corpus (₹)</Label>
                <Input type="number" value={currentCorpus} onChange={(e) => setCurrentCorpus(e.target.value)} placeholder="e.g. 100000" className="mt-1" />
              </div>
              <div>
                <Label className="font-medium text-gray-700">Annuity Purchase at Retirement (%)</Label>
                <Input type="number" value={annuityPurchasePct} onChange={(e) => setAnnuityPurchasePct(e.target.value)} placeholder="min 40" className="mt-1" />
              </div>
              <div>
                <Label className="font-medium text-gray-700">Annuity Annual Rate (%)</Label>
                <Input type="number" value={annuityRatePct} onChange={(e) => setAnnuityRatePct(e.target.value)} placeholder="e.g. 6" className="mt-1" />
              </div>
            </div>
        ...
        {/* --- Info section (plain text paragraphs) --- */}
        <div className="max-w-3xl mx-auto space-y-3 text-gray-700">
          <h2 className="text-xl font-semibold text-gray-800">About the NPS Calculations Used in This Tool</h2>
          <p>
          ...
          <p>
            At retirement, at least <strong>40% of the total corpus</strong> must be used to buy an annuity, and the remaining can be taken as a lump sum (currently exempt from tax).
          </p>
          <p>
            <strong>What is an Annuity?</strong> An annuity is a product you buy at retirement that pays a fixed monthly pension. The pension depends on the
            annuity purchase amount, the annuity interest rate, and the plan chosen (e.g., lifetime, with/without return of purchase price, spouse pension, etc.).
          </p>
          <p>
            <strong>Example:</strong> If Basic Pay = ₹50,000 and DA = 50% → Basic + DA = ₹75,000 → Employee NPS: 7,500/month; Employer NPS: 10,500/month;
            Total: <strong>₹18,000/month</strong>.
          </p>
        </div>
